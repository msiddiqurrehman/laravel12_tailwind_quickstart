<x-admin-layout>
    <div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6">
        <!-- Breadcrumb Start -->
        <div x-data="{ pageName: `Designation List`, pages: [
        ...
        <div class="space-y-5 sm:space-y-6">
            <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="flex justify-between px-5 py-4 sm:px-6 sm:py-5">
                    ...
                    <a href="{{ route('admin.designations.create') }}" class="text-theme-lg font-medium text-gray-500 dark:text-white/90">
                            ...
                            <table id="to-data-table" class="display responsive">
                                ...
                                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-5 py-4 sm:px-6">
                                            <div class="flex items-center">
                                                <a href="{{ route('admin.designations.edit', $item) }}" title="Edit" class="p-1 me-2.5 text-yellow-500">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="size-4">
                                                        <path fill-rule="evenodd" d="M11.013 2.513a1.75 1.75 0 0 1 2.475 2.474L6.226 12.25a2.751 2.751 0 0 1-.892.596l-2.047.848a.75.75 0 0 1-.98-.98l.848-2.047a2.75 2.75 0 0 1 .596-.892l7.262-7.261Z" clip-rule="evenodd" />
                                                    </svg>                                              
                                                </a>
                                                <form action="{{ route('admin.designations.destroy', $item) }}" method="post" id="{{ 'form-delete-'.$item->id }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a href="javascript:void(0);" title="Delete"
                                                        @click="showConfirmDeleteModal = true; formToDeleteId='{{ 'form-delete-'.$item->id }}'; confirmModalMsg='Are you sure to delete designation <span class= \'font-medium \'>{{ $item->title }}</span>?'" 
                                                        class="text-red-600"
                                                    >
