@props(['name', 'label' => '', 'isMultiple' => false, 'oldImagePath' => null, 'fileAccept' => 'image/*', 'isImageUpload' => true ])
<!-- Image upload -->
<div  x-data="{ 
                images: [], 
 ...
 >
    ...
    <div class="icons mt-10">
        <label class="cursor-pointer flex mb-1.5 text-sm font-medium text-gray-700 dark:text-gray-400">
            ...
            <input  type="file" 
                    ...
                    accept="{{ $fileAccept }}"
                    @change="images = Array.from($event.target.files).map(
                                        file => ({
    ...
    <div id="preview" class="flex">
        <template x-for="(image, index) in images" :key="index">
            <div class="relative w-32 h-32 my-4 object-cover rounded ">
                {{-- Show image if preview is true (it's an image) --}}
                ...
                {{-- Show image if preview is FALSE (it's NOT an image) --}}
                <div x-show="!image.preview" class="relative w-32 h-32 object-cover rounded">
                    ...
                    <svg class="fill-current  w-32 h-32 ml-auto pt-1 text-gray-400"
    ...
    <div x-show="showOldImage" class="relative w-32 my-4 object-cover rounded">
        ...
        @else
            <a href="{{ $oldImagePath }}" target="_blank">
                <svg class="fill-current w-32 h-32 ml-auto pt-1 text-gray-400"
                    xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                    <path d="M15 2v5h5v15h-16v-20h11zm1-2h-14v24h20v-18l-6-6z" />
                </svg>
                <span class="absolute top-15 start-6.5 dark:text-white/90">Download</span>
            </a>
        @endif
        {{ $slot }}
    </div>
